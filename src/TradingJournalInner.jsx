...
const TradingJournalInnerImpl = memo(function TradingJournalInnerImpl({ onLockNow, onChangePin }) {
  ...
  // ── Finanzas Personales (pestaña independiente del trading) ──
  // Gastos e ingresos del día a día, sin relación con las cuentas de trading.
  // Cada registro: { id, nombre, fecha: "YYYY-MM-DD", valor, categoria }.
  // El "mes" se deriva siempre de la fecha, no se guarda por separado.
  ...
  // ── Finanzas: constantes, helpers de CRUD y agrupación por período ──
  // Antes eran arrays fijos acá mismo; ahora son alias de las listas editables
  // (gastoCatsList/ingresoCatsList, arriba) para no tener que tocar cada lugar
  // del archivo que ya las usaba por este nombre.
  const GASTO_CATS = gastoCatsList;
  const INGRESO_CATS = ingresoCatsList;
  const MONEDAS = ["USD", "CLP"];
  // CLP no usa decimales y tiene su propia convención de agrupación — formatear
  // un valor en CLP con la misma función `money()` (pensada para USD) mostraría
  // "$1,234.56" para lo que en realidad es "$1.234" sin decimales. Un solo
  // helper decide el formato según la moneda del registro.
  const finMoneyFmt = (value, moneda) => moneda === "CLP"
    ? new Intl.NumberFormat("es-CL", { style: "currency", currency: "CLP", maximumFractionDigits: 0 }).format(value)
    : money(value);
  // MESES_FIN vivía duplicado acá (era un calco de MONTHS_FULL, ya importado
  // de constants.js) — se saca y se reusa MONTHS_FULL/MONTHS_SHORT directamente.
